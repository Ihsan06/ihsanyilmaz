// Die Bildmarke: das Delta-Monogramm (A als Silhouette, Y im Negativraum,
// I als Mittelstamm). Als Pfad eingebettet statt als Datei geladen — so
// faerbt sie sich mit currentColor und braucht keinen zweiten Abruf.
export const BILDMARKE_PFAD =
...
export function Bildmarke({ size = 28, className = "" }: { size?: number; className?: string }) {
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} className={className}
      ...
      <path d={BILDMARKE_PFAD} />
    </svg>
  );
}

// Bildmarke plus Schriftzug — der Kopf- und Fusszeilen-Auftritt der Website.
export default function Logo({
  size = 30, name = true, className = "",
}: { size?: number; name?: boolean; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      <Bildmarke size={size} className="shrink-0" />
      <span className="flex flex-col leading-none">
        <span className="font-display font-bold tracking-tight"
          style={{ fontSize: size * 0.72 }}>AIY</span>
        {name && (
          <span className="font-medium text-[var(--fg-muted)] uppercase"
            style={{ fontSize: size * 0.263, letterSpacing: "0.14em", marginTop: size * 0.1 }}>
            Ihsan Yilmaz
          </span>
        )}
      </span>
