  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <Zahl icon={<CalendarClock size={18} />} label="Eingeplant" wert={geplant.length} />
        <Zahl
          icon={<Heart size={18} />}
        ...
        {naechster ? (
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-[var(--fg)] font-medium">
              {naechster.format === "story" ? "Story" : "Beitrag"}
            </span>
            <span className="text-[var(--fg-muted)] text-sm truncate max-w-md">
              {(naechster.text || "").split("\n")[0]}
            </span>
            <a href="/admin/planen" className="text-sm font-medium hover:opacity-80 ml-auto"
              style={{ color: "var(--accent)" }}>zur Warteschlange →</a>
          </div>
        ) : (
          <p className="text-[var(--fg-muted)] text-sm">
            Nichts eingeplant.{" "}
            <a href="/admin/content" className="font-medium hover:opacity-80"
              style={{ color: "var(--accent)" }}>Jetzt einen Beitrag bauen →</a>
          </p>
        )}
      </div>
...
function Zahl({ icon, label, wert }: { icon: React.ReactNode; label: string; wert: number | null }) {
  return (
